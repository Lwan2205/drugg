...
const ProductList = () => {
    ...
    const productsPerPage = 1; // Số sản phẩm mỗi trang
    ...
    // Gọi API để lấy danh sách sản phẩm dựa trên productId
    useEffect(() => {
        const fetchProductsByproduct = async () => {
            try {
                const response = await fetch(`http://localhost:8000/api/products/product/${productId}?page=${currentPage}&limit=${productsPerPage}`, {
                    method: 'GET',
                    credentials: 'include',
                });
        ...
        fetchProductsByproduct(); // Gọi hàm lấy sản phẩm khi component được render
    }, [productId, currentPage]); // Chạy lại khi productId hoặc currentPage thay đổi
